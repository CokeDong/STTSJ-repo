...
void split(std::string s, std::string delim, std::vector<std::string>* ret)
{
	...
	while (index != std::string::npos) // not not success   std::string::npos is -1 actually
	{
		...
		if (stmp.length() != 0) ret->push_back(stmp); // std::vector不可以push_back NULL字符
		last = index + delim.size();
		index = s.find_first_of(delim, last);
		//cout << index << endl;
	}
	if (last < s.size()) // 行尾无delim
	{
		std::string stmp = s.substr(last, index - last);
		if (stmp.length() != 0) ret->push_back(stmp);
	}
	//if (index - last > 0) // wrong judgement!!  行尾无delim index == std::string::npos
	//{
...
void split2(std::string s, std::string delim, std::vector<std::string>* ret)
{
	...
	if (s.size() - last > 0) // index == std::string::npos 行尾无delim
	{
...
void split3(std::string s, std::string delim, std::vector<std::string>* ret)
{
	...
	while (index != std::string::npos) // not not success   std::string::npos is a big num
	{
		std::string stmp = s.substr(last, index - last);
		if (stmp.length() != 0) {
			ret->push_back(stmp); // std::vector不可以push_back NULL字符
...
void GetSample_Filtering(std::vector<STTrajectory> &dataptr, std::vector<size_t> &taskSet1, std::vector<size_t> &taskSet2, int sizeP, int sizeQ) {
